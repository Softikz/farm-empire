...
async def sell_crop(call: types.CallbackQuery):
    ...
    # Проверяем, первая ли это продажа
    is_first_sell = user.get("tutorial_complete", False) and not user.get("first_sell_done", False)
    user["first_sell_done"] = True

    text = f"✅ Продано!\n\n{crop} x{amount}\n💰 Получено: {total} монет"
    
    # Добавляем обучение от Пиги после первой продажи
    if is_first_sell:
        text += (
            "\n\n" + "="*40 + "\n"
            "🐷 Пиги: Отлично! Первая продажа!\n"
            "Вот несколько советов для успеха:\n\n"
            "📚 Рекомендую посетить библиотеку в городе\n"
            "и изучить культуры - тут много полезной\n"
            "информации про урожайность и доход!\n\n"
            "👑 Не забудь про завхоза СНТ - там можно\n"
            "получить награды за выполнение заданий!\n\n"
            "🏪 Магазин поможет расширить ферму\n"
            "и купить новые семена.\n\n"
            "Удачи, фермер! 🌾"
        )

    await call.message.edit_text(text, reply_markup=city_kb())
